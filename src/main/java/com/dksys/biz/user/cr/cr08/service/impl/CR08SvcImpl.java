...
public class CR08SvcImpl implements CR08Svc {
	...
	public int grid1_selectCount(Map<String, String> paramMap) {
		return cr08Mapper.grid1_selectCount(paramMap);
	}

	// 그리드 리스트
	@Override
	public List<Map<String, String>> grid1_selectList(Map<String, String> paramMap) {
		return cr08Mapper.grid1_selectList(paramMap);
	}

	// 팝업 입력대상 검색
	@Override
	public List<Map<String, String>> select_insert_target_modal(Map<String, String> paramMap) {
		return cr08Mapper.select_insert_target_modal(paramMap);
	}
	
	// 수정화면 정보
	@Override
	public Map<String, String> select_cr08_Info(Map<String, String> paramMap) {
		return cr08Mapper.select_cr08_Info(paramMap);
	}
	
	// 수정화면 상세정보
	@Override
	public List<Map<String, String>> select_cr08_Info_Dtl(Map<String, String> paramMap) {
		return cr08Mapper.select_cr08_Info_Dtl(paramMap);
	}
	
	//DATA INSERT
	@Override
	public int insert_cr08(Map<String, String> paramMap, MultipartHttpServletRequest mRequest) throws Exception {
		...
		//데이터 처리 시작
		int fileTrgtKey = cr08Mapper.select_cr08_SeqNext(paramMap);
		paramMap.put("fileTrgtKey", Integer.toString(fileTrgtKey));

		String newMNGM_NO = cr08Mapper.select_cr08_Next_MNGM_NO(paramMap);
		paramMap.put("sellBillNo", newMNGM_NO);
		
		//마스터입력
		int result = cr08Mapper.insert_cr08(paramMap);
	    ...
	    for (Map<String, String> dtl : dtlParam) {
	    	...
	    	if ("I".equals(dataChk)) {
				dtl.put("sellDcsnNo", dtl.get("sellDcsnNo").toString());
				dtl.put("ordrsNo", dtl.get("ordrsNo").toString());
				dtl.put("clmnPlanSeq", dtl.get("clmnPlanSeq").toString());
				dtl.put("sellBillDtlAmt", dtl.get("billAmt").toString());
				
				//데이터 처리
				cr08Mapper.insert_cr08_Dtl(dtl);
				cr08Mapper.update_cr08_Conf(dtl);
				//i++;
	    	}
	    }
		//데이터 처리 끝
	...
	//DATA UPDATE
	@Override
	public int update_cr08(Map<String, String> paramMap, MultipartHttpServletRequest mRequest) throws Exception {
		//Gson gson = new Gson();
		Gson gsonDtl = new GsonBuilder().disableHtmlEscaping().create();
		Type dtlMap = new TypeToken<ArrayList<Map<String, String>>>(){}.getType();
		...
		//데이터처리 시작
		//마스터 수정
		int result = cr08Mapper.update_cr08(paramMap);
	    ...
	    for (Map<String, String> dtl : dtlParam) {
	    	...
	    	if ("U".equals(dataChk)) {
				//데이터 처리
				cr08Mapper.update_cr08_Dtl(dtl);
	    	} 
	    }
		//데이터 처리 끝
	...
	//DATA DELETE
	@Override
	public int delete_cr08(Map<String, String> paramMap) throws Exception {
		//---------------------------------------------------------------
		...
		result = cr08Mapper.delete_cr08_Dtl_All(paramMap);
		result = cr08Mapper.delete_cr08(paramMap);
		result = cr08Mapper.update_cr08_Del(paramMap);

...
import com.dksys.biz.user.cr.cr51.mapper.CR51Mapper;
...
public class CR51SvcImpl implements CR51Svc {

	@Autowired
	CR51Mapper cr51Mapper;
	...
	public int selectPfuListCount(Map<String, String> paramMap) {
		return cr51Mapper.selectPfuListCount(paramMap);
	}

	@Override
	public List<Map<String, String>> selectPfuList(Map<String, String> paramMap) {
		return cr51Mapper.selectPfuList(paramMap);
	}
	
	
	@Override
	public List<Map<String, String>> selectPFUAreaItemList(Map<String, String> paramMap) {
		List<Map<String, String>> result = cr51Mapper.selectPFUAreaItemList(paramMap);
		return result;
	}
	...
	public List<Map<String, String>> selectPFUAreaRetriveList(Map<String, String> paramMap) {
		return cr51Mapper.selectPFUAreaRetriveList(paramMap);
	}
...
//	    return cr51Mapper.selectPfuClobInfo(paramMap);
//	}
	
	
	@Override
	public List<Map<String, String>> selectStdPfuClobInfo(Map<String, String> paramMap) {
	    return cr51Mapper.selectStdPfuClobInfo(paramMap);
	}
	...
	public int insertPfu(Map<String, String> paramMap, MultipartHttpServletRequest mRequest) throws Exception {
		...
		//첨부 화일 권한체크  끝 
		//---------------------------------------------------------------  
		int result = cr51Mapper.insertPfu(paramMap);
		String  newPfuSeq = paramMap.get("fileTrgtKey");

		
		String sysCreateDttm =  cr51Mapper.selectSystemCreateDttm(paramMap);
        int newSeq = 0;
		for (int i = 1; i <= 6; i++) {
...
//		Gson gson = new Gson();
		...
		int result = cr51Mapper.updatePfu(paramMap);

		int deleteCnt = cr51Mapper.deletePfuAreaAll(paramMap);	//동록된 사양 일괄 삭제 처리
...
//			            detailMap.put("udtDttm", "");
		            ...
		            result += cr51Mapper.insertPfuArea(detailMap);	//삭제하고 전부 다시 등록처리함. (일련번호 또는 정렬순서)
		        }
		    }
		}
		...
		deleteCnt += cr51Mapper.deletePfuSalesCdAll(paramMap);	//동록된 SalesCd 일괄 삭제 처리
	  ...
	  public int deletePfuNo(Map<String, String> paramMap) throws Exception {
			...
			String deleteCheck =  cr51Mapper.selectPfuDeleteCheck(paramMap);
		    if ("Y".equals(deleteCheck)) {
		    	//한명이라도 결재가 완료되면 삭제는 불가능합니다.
		    	throw new Exception("결재완료건이 있습니다. 확인바랍니다."); 
		    }
			
		    int result = cr51Mapper.deletePfuNo(paramMap);
//		    //상세내역 삭제
		    result += cr51Mapper.deletePfuAreaAll(paramMap);
		...
		// PFU복사부분 시작
		@Override
		public int copy_cr51(Map<String, String> paramMap, MultipartHttpServletRequest mRequest) throws Exception {
			Gson gsonDtl = new GsonBuilder().disableHtmlEscaping().create();
			Type dtlMap = new TypeToken<ArrayList<Map<String, String>>>(){}.getType();
			HashMap<String, String> param = new HashMap<>();
			param.put("userId", paramMap.get("userId"));
		    ...
		    for (Map<String, String> dtl : dtlParam) {
		    	...
		    	if ("I".equals(dataChk)) {
					//데이터 처리
//		    		resultDel = cr51Mapper.delete_cr51_master(dtl);
//		    		resultDel = cr51Mapper.delete_cr51_detail(dtl);
					result += cr51Mapper.copy_cr51_master(dtl);	//TB_CR51M01 마스터 정보 복사
					result += cr51Mapper.copy_cr51_detail(dtl); //TB_CR51D01 상세내역 복사
		    	} 
		    }
			//데이터 처리 끝
			return result;
		...
		public int selectPfuCopyTargetListCount(Map<String, String> paramMap) {
			return cr51Mapper.selectPfuCopyTargetListCount(paramMap);
		}

		@Override
		public List<Map<String, String>> selectPfuCopyTargetList(Map<String, String> paramMap) {
			return cr51Mapper.selectPfuCopyTargetList(paramMap);
		}
		

	    @Override
		public int selectPfuIsThereListCount(Map<String, String> paramMap) {
			return cr51Mapper.selectPfuIsThereListCount(paramMap);
		}

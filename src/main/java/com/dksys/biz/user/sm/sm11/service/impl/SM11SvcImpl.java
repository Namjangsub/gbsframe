...
//			result = sm11Mapper.etcPchsOrderMasterReport(dtl);
	...
	public int createContractBill(Map<String, String> paramMap) throws Exception {
		...
		int contractBillExistChk = sm11Mapper.selectContractBillExistChk(paramMap);
		if (contractBillExistChk > 0) {
			throw new RuntimeException("이미 등록된 기타매입이 존재합니다. 다시 확인해 주세요.");
		}
	...
	public int deleteContractBill(Map<String, String> paramMap) throws Exception {
		...
		int contractBillExistChk = sm11Mapper.selectContractBillExistChk(paramMap);
		if (contractBillExistChk == 0) {
			throw new RuntimeException("이미 취소되었습니다. 다시 확인해 주세요.");
		}
		//기타매입 마스터 삭제

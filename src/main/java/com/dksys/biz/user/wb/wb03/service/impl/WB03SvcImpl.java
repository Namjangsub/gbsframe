...
public class WB03SvcImpl implements WB03Svc {
  ...
  public int insertBom(Map<String, String> paramMap, MultipartHttpServletRequest mRequest) throws Exception {
		...
		//첨부 화일 권한체크  끝 
		//---------------------------------------------------------------  
		//String  newPrjctSeq = String.valueOf(wb03Mapper.selectPrjctSeqNext(paramMap));
		//paramMap.put("prjctSeq", newPrjctSeq);
		//int result = wb03Mapper.insertBom(paramMap);
		int result = 200;
	    ...
	    for (Map<String, String> dtl : bomList) {
	    	...
	    	if ("I".equals(dtaChk)) {
	    		wb03Mapper.insertBom(dtl);
	    	} else if ("U".equals(dtaChk)) {
	    		wb03Mapper.updateBom(dtl);
	    	} else if ("D".equals(dtaChk)) {
	    		wb03Mapper.deleteBomMatrAll(dtl);
	    		wb03Mapper.deleteBom(dtl);
	    	}
	    }
	    ...
	    for (Map<String, String> dtl : matrList) {
	    	...
	    	String dtaChk = dtl.get("dtaChk").toString();
	    	/* "dtaChk" 값을 확인하여
	    	 * "I"인 경우 wb03Mapper.insertBomMatr(dtl)을 호출하여 프로젝트 세부정보를 삽입 */
	    	if ("I".equals(dtaChk)) {
	    		wb03Mapper.insertBomMatr(dtl);
	    	} 
	    }
...
//	Gson gson = new Gson();
	...
	int result = 200;//wb03Mapper.updateBom(paramMap);
    	//  wb03Mapper.updateBom(paramMap)을 호출하여 paramMap을 사용하여 프로젝트를 업데이트하고 그 결과를 result 변수에 저장.
    ...
    for (Map<String, String> dtl : bomList) {
    	...
    	String dtaChk = dtl.get("dtaChk").toString();
    	/* "dtaChk" 값을 확인하여
    	 * "I"인 경우 wb03Mapper.insertBomMatr(dtl)을 호출하여 프로젝트 세부정보를 삽입하고,
    	 * "U"인 경우 wb03Mapper.updateBomMatr(dtl)을 호출하여 프로젝트 세부정보를 업데이트하고,
    	 * "D"인 경우 * wb03Mapper.deleteBomMatr(dtl)을 호출하여 프로젝트 세부정보를 삭제.		 */
    	if ("I".equals(dtaChk)) {
    ...
    for (Map<String, String> dtl : matrList) {
      ...
      if ("I".equals(dtaChk)) {
	        ...
	        wb03Mapper.updateBomMatr(dtl);
	      } else if ("D".equals(dtaChk)) {
	        wb03Mapper.deleteBomMatr(dtl);
	      }
    }
	//---------------------------------------------------------------  
	//첨부 화일 처리 시작 
  ...
  public int deleteBomAll(Map<String, String> paramMap) throws Exception {
	  ...
	  int result = wb03Mapper.deleteBomMatrAll(paramMap);
	  result = wb03Mapper.deleteBomAll(paramMap);

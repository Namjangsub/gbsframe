...
/* Global Chat Toast Notification */
(function() {
    // Force Custom Toast CSS
    var customToastStyle = document.createElement('style');
    customToastStyle.innerHTML = `
        ...
        #global-chat-notification .gcn-header {
            padding: 10px 15px;
            background: #f1f5f9;
            color: #334155;
            font-weight: 600;
        ...
        #global-chat-notification .gcn-body {
            padding: 0;
            max-height: 400px;
            overflow-y: auto;
        }
        #global-chat-notification .gcn-message {
            padding: 12px 15px;
        ...
        #global-chat-notification .gcn-message:last-child {
            border-bottom: none;
        }
        #global-chat-notification .gcn-message:hover {
            background: #f8fafc;
        }
        #global-chat-notification .gcn-message strong {
            color: #0f172a;
            display: block;
            margin-bottom: 2px;
        }
        #global-chat-notification .gcn-close {
            cursor: pointer;
            color: #94a3b8;
            font-size: 16px;
        }
        #global-chat-notification .gcn-close:hover {
            color: #ef4444;
        }
    `;
    ...
    // 2. Ensure jwt/userId is available
    if (typeof jwt === 'undefined' || !jwt || !jwt.userId) {
        return;
    }
    ...
    function initGlobalChat() {
        if (typeof SockJS === 'undefined' || typeof Stomp === 'undefined') {
            loadScript('/static/js/websocket/sockjs.min.js', function() {
                loadScript('/static/js/websocket/stomp.min.js', function() {
                    connectGlobalChat();
                });
            });
        } else {
    ...
    function connectGlobalChat() {
        ...
        globalStompClient.connect({}, function(frame) {
            globalStompClient.subscribe('/topic/chat-list/' + jwt.userId, function(message) {
                try {
                    var payload = JSON.parse(message.body);
                    handleGlobalChatMessage(payload);
                } catch (e) {
                    // console.error(e);
                }
            });
    ...
    function handleGlobalChatMessage(payload) {
        // 1. Check if user is currently in the specific chat room via SessionStorage
        try {
            var storedParams = sessionStorage.getItem("chatParams");
            if (storedParams) {
                var currentChat = JSON.parse(storedParams);
                if (currentChat.roomId && String(currentChat.roomId) === String(payload.roomId)) {
                    return;
                }
            }
        } catch(e) {}
        ...
        // 2. De-duplication: prevent processing same message multiple times
        var msgId = payload.messageId;
        var now = new Date().getTime();
        var lastId = localStorage.getItem("global_chat_last_msg_id");
        var lastTime = localStorage.getItem("global_chat_last_time");
        if (lastId === String(msgId) && lastTime && (now - Number(lastTime) < 5000)) {
            return;
        }

        localStorage.setItem("global_chat_last_msg_id", String(msgId));
        localStorage.setItem("global_chat_last_time", String(now));

        resolveGlobalToastData(payload, function(resolved) {
            if (!resolved) return;
            renderToast(resolved);
        });
    }

    function resolveGlobalToastData(payload, callback) {
        var senderName = payload && payload.senderName ? String(payload.senderName).trim() : "";
        var content = payload && (payload.bodyPreview || payload.message || payload.body || payload.content) ? String(payload.bodyPreview || payload.message || payload.body || payload.content).trim() : "";
        var sentAtVal = payload && (payload.sentAt || payload.timestamp) ? String(payload.sentAt || payload.timestamp).trim() : "";
        var senderId = payload && (payload.senderUserId || payload.senderId) ? String(payload.senderUserId || payload.senderId).trim() : "";

        if (jwt && jwt.userId && senderId && String(senderId) === String(jwt.userId)) {
            callback(null);
            return;
        }
        ...
        var messageId = payload && payload.messageId ? payload.messageId : "";
        if (!roomId || !messageId) {
            callback(null);
            return;
        }

        postAjax("/chat/selectChatToastInfo", { roomId: String(roomId), messageId: String(messageId) }, null, function (data) {
            var info = data && data.result ? data.result : null;
            var nm = info && info.senderName ? String(info.senderName).trim() : "";
            var bd = info && info.bodyPreview ? String(info.bodyPreview).trim() : "";
            var st = info && info.sentAt ? String(info.sentAt).trim() : "";
            var sid = info && info.senderUserId ? String(info.senderUserId).trim() : senderId;
            if (jwt && jwt.userId && sid && String(sid) === String(jwt.userId)) {
                callback(null);
                return;
            }
            if (!nm || !bd) {
    ...
    function renderToast(payload) {
        ...
        if ($toast.length === 0) {
            $('body').append(`
                <div id="global-chat-notification">
                    <div class="gcn-header">
                        <span><i class="fa fa-comment-dots" style="color:#1d4ed8; margin-right:5px;"></i> 새 메시지</span>
                        <span class="gcn-close"><i class="fa fa-times"></i></span>
                    </div>
                    <div class="gcn-body"></div>
                </div>
            `);
        ...
        var content = payload.message || payload.body || "";
        ...
        var msgHtml = `
            <div class="gcn-message" data-room-id="${safeRoomId}" data-room-name="${safeRoomName}" style="cursor:pointer;">
                <div style="display:flex; justify-content:space-between; align-items:center;">
                    <strong>${senderName}</strong>
                    <span style="font-size: 11px; color: #64748b;">${timeLabel}</span>
                </div>
                <div style="font-size:13px; margin-top:2px; color: #334155;">
                    ${safeContent.length > 30 ? safeContent.substring(0, 30) + '...' : safeContent}
                </div>
            </div>
        `;

        $toast.find('.gcn-body').empty().append(msgHtml);
        $toast.show();

        var bodyDiv = $toast.find('.gcn-body')[0];
        try { bodyDiv.scrollTop = bodyDiv.scrollHeight; } catch(e){}

        if (window.globalToastCloseTimer) {
    ...
    // Listen for dismiss events from other tabs
    window.addEventListener('storage', function(e) {
        if (e.key === 'global_chat_dismiss' && e.newValue) {
            try {
                var dismissData = JSON.parse(e.newValue);
                var currentMsgId = localStorage.getItem("global_chat_last_msg_id");
                if (dismissData.messageId === currentMsgId) {
                    dismissGlobalToast(null); // Don't re-trigger storage event
                }
            } catch(err) {}
        }
    });

    // Auto-Close Logic on User Interaction
    var interactHandler = function(e) {
        var $toast = $('#global-chat-notification');
        if ($toast.length === 0 || !$toast.is(':visible')) return;

        if ($(e.target).closest('#global-chat-notification').length > 0) return;
        if (window.globalToastCloseTimer) return;
    ...
    $(document).on('click', '.gcn-message', function(e) {
        ...
        if (roomId) {
            ...
            sessionStorage.setItem("chatParams", JSON.stringify(chatParams));
            window.location.href = "/static/html/chat/chat01/CHAT02M01.html";
        }
    });

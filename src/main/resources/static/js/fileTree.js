...
//			const response = await fetch(`/admin/cm/cm08/fileDownloadAuth?fileKey=${fileKey}&userId=${jwt.userId}&coCd=${coCd}`);
	...
	function callApprovalWorking(todoKey, callPgm = ''){
		...
		//필요시 프로그램 에 따라 분기 처리
		if (row.teamManager == '평가' || (callPgm == 'PM5102P01' && row.todoDiv2CodeId == 'TODODIV2200')) { // 팀장 결재시
			let chkFlag = false;
			...
			} else if (callPgm == 'PM5102P01' && row.todoDiv2CodeId == 'TODODIV2200') {
...
//								alert("공유 확인되었습니다.");
			...
			if (amDocId) {
				...
				openThirdModal("/static/html/user/wb/wb20/WB2001P01.html", 730, 460, "", paramObj, function(data){

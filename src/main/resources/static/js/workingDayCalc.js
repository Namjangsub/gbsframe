...
//  const startDate = "2024-12-29"
	...
	var eventHolidays = [   "20140910", "20150929", "20160210", "20170130", "20171006", 
							...
							"20230529", "20240212", "20240410", "20240506", 
...
//		while (workingDays < chkDuration) {
		do {
			...
			if (lunaChkYear != chkYear) {  //년도가 바뀌면 음력설전날을 추가 하는 배열 생성
				 ...
				 **************************
				 */
				calendar.setLunarDate(chkYear,1 ,1 , false) //음력설날 양력으로 변환 20230101 --> 0122
				...
				for (const lunaday of lunaHoliTempdays) { //음력날자에 해당하는 양력휴일 추구
					calendar.setLunarDate(chkYear, parseInt(lunaday.slice(0, 2)), parseInt(lunaday.slice(2, 4)), false);
					//solarCalendar: {year: 2023, month: 1, day: 22}
				...
				var combinedHolidays = eventHolidays.concat(solarHolidays.map(date => chkYear + date)).sort();
					
				yearHolidays = combinedHolidays.filter(function(date) {
					return date.startsWith(chkYear);
				});
				/**************************
				 * 휴일테이블만들기 End
				 **************************

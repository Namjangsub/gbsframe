				...
				postAjaxSync("/user/wb/wb20/insertApprovalLine", paramMap, null, function(data){
					if(data.resultCode == 200){
						...
						if( todoYn == "Y" || todoCfOpn != '') {		//모든 결재요청이 완료되면 카톡 전송
							...
							// PM51/PM07/PM08/... 순차결재: 중간 결재자가 결재의견 없이 승인하면 sendTodoFinal()이 호출되지 않아
...
//			    });  
	...
	function sendTodoFinal(param) {
		...
		var isSeqTarget = param.todoDiv2CodeId === 'TODODIV2190' || param.todoDiv2CodeId === 'TODODIV2191'
